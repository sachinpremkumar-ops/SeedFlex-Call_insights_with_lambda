...
from src.Tools.Storage_Agent_Tools import insert_data_all, move_file_to_processed, make_embeddings_of_transcription
...
def Speech_Model(state:AgentState) -> AgentState:
    ...
    try:
        messages = state.get('messages', [])
        # Keep recent messages but limit to avoid token bloat
        
        messages =  [SystemMessage(content=Speech_Model_Template)] + messages
        ...
        if isinstance(response, Command):
            # Create new state with only the latest response
            new_state = {**state, "messages": [response]}
            # Add tool response messages if any
            if "messages" in response.update:
                new_state["messages"] = new_state["messages"] + response.update["messages"]
...
def Summarization_Model(state:AgentState):
    ...
    try:
        messages = state.get('messages', [])
        messages =  [SystemMessage(content=Summarization_Model_Template)] +messages
        Summarization_Model_With_Tools = model_gpt4.bind_tools(Summarization_Model_Tools)  
...
def Insights_Model(state:AgentState):
    try:
        ...
        messages =  [SystemMessage(content=Speech_Model_Template + f"Transcription: {transcription}\n\nSummary: {summary}\n\nTranslation: {translation}" )]+ messages
        ...
        if isinstance(response, Command):   
            ...
            execution_time = (end_time - start_time).total_seconds()
            default_state = {**state, "messages": [response]}
